...
class RegistrationPage(QWidget):
    ...
    def show_registration_results(self):
        ...
        for item_text in self.shared_items:  # 장바구니와 공유되는 아이템 리스트 사용
            ...
            btn_apply_registration.clicked.connect(self.apply_registration)
    ...
    def apply_registration(self):
        ...
        selected_items = self.list_registration_results.selectedItems()
        for item in selected_items:
            item_text = item.text()
            print(f"강의를 수강신청합니다: {item_text}")

            # 수강신청한 강의를 장바구니 아이템 리스트에 추가
            self.cart_items.append(item_text)
            # 수강신청한 강의를 공유 아이템 리스트에도 추가
    ...
    def delete_cart_item_for_registration_item(self, item):
        # Delete 버튼 클릭 시 해당 아이템 삭제
        item_text = item.text()
        print(f"수강신청에서 아이템을 삭제합니다: {item_text}")
